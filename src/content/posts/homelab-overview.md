...
next to the router, switch, a NAS enclosure, and a printer that isn't part of the homelab but shares the shelf
anyway.
...
That per-group isolation is a deliberate trade. It costs more RAM and disk overhead than cramming everything
into one Docker host, but it keeps the blast radius small when something breaks, and each VM can be rebuilt
...
Admin tools and anything with a history of being targeted when exposed directly to the internet stay
reachable only over a private VPN mesh instead of getting a public DNS entry. Only things that genuinely need
to be reachable by anonymous visitors or locked-down client devices like smart TVs and game consoles get
exposed publicly.
...
- A **documentation repo**, versioned in git, recording every VM, every service's config, and every
  troubleshooting session, so six months from now I can find out why a given port or mount path is set up the
  way it is instead of re-deriving it from scratch.

## What's running on it

At this point the homelab hosts, among other things, a Minecraft server with its own control panel, a phpBB
forum, a wiki, a Matrix chat homeserver, a full media stack for acquisition, organization, and playback,
photo hosting, a federated social network node, household inventory tracking, calendar/contacts sync, and a
TTRPG playtest web app I'm building myself. This blog is the newest addition, running as just another
container in that same stack.

## What's next

This is a living system. Future posts here will cover specific builds as they happen: new hardware, service
migrations, and the troubleshooting that comes with running all of this on hardware I bought and diagnose
myself.
